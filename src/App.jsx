...
async function fetchProducts() {
  if (!SUPABASE_URL || !SUPABASE_KEY) throw new Error("Credenziali Supabase mancanti");
...
const css = `
  ...
  * { box-sizing:border-box; margin:0; padding:0; }
  ...
  .ft-logo-title span { color:#6C63FF; }
  ...
  .ft-add-btn { width:100%; background:#6C63FF; color:#fff; border:none; border-radius:10px; padding:12px; font-weight:600; font-size:14px; cursor:pointer; font-family:'Inter',sans-serif; transition:all 0.15s; display:flex; align-items:center; justify-content:center; gap:8px; }
  .ft-add-btn:hover { background:#7C75FF; transform:translateY(-1px); }
  ...
  .ft-page-title svg { color:#6C63FF; opacity:0.8; }
  ...
  .ft-spin { animation:spin 1s linear infinite; }
  ...
  .ft-cat-bar-fill { height:100%; border-radius:10px; }
  ...
  .ft-tooltip-label { color:#6060a0; font-size:10px; text-transform:uppercase; margin-bottom:2px; }
  ...
  .ft-row::before { content:''; position:absolute; left:0; top:20%; bottom:20%; width:2.5px; border-radius:0 2px 2px 0; background:var(--accent,#333); }
  ...
  .ft-overlay { position:fixed; inset:0; background:rgba(0,0,0,0.8); backdrop-filter:blur(16px); z-index:100; display:flex; align-items:center; justify-content:center; }
  .ft-modal { background:#0F0F1A; border:1px solid rgba(255,255,255,0.1); border-radius:20px; padding:36px; width:500px; max-width:93vw; max-height:92vh; overflow-y:auto; position:relative; }
  ...
  .ft-save-btn { width:100%; background:#6C63FF; color:#fff; border:none; border-radius:10px; padding:13px; font-weight:600; font-size:14px; cursor:pointer; font-family:'Inter',sans-serif; margin-top:4px; transition:all 0.15s; display:flex; align-items:center; justify-content:center; gap:8px; }
  ...
  .ft-mobile-nav-item.active svg { color:#6C63FF; }
  .ft-mobile-nav-item.active .ft-mobile-label { color:#6C63FF; }
  .ft-fab { display:none; position:fixed; bottom:84px; right:18px; background:#6C63FF; color:#fff; border:none; border-radius:50%; width:52px; height:52px; cursor:pointer; box-shadow:0 8px 24px rgba(108,99,255,0.4); z-index:91; align-items:center; justify-content:center; }
...
function StatCard({ label, val, sub, color, Icon }) {
  return (
    <div className="ft-stat">
      <div style={{ position:"absolute", inset:0, background:`radial-gradient(ellipse at 5% 90%, ${color}18 0%, transparent 60%)`, pointerEvents:"none" }} />
      <div className="ft-stat-icon" style={{ background:`${color}20` }}>
...
function StatusPill({ status }) {
  ...
  return (
    <span className="ft-status-pill" style={{ background:`${color}20`, color, border:`1px solid ${color}40` }}>
      {label}
    </span>
  );
}
  ...
  return (
    <div className="ft-row" style={{ "--accent": accent }} onClick={() => onClick(item)}>
      <div>
        ...
        <div className="ft-row-sub"><Tag size={9} />{item.category} · {item.purchase_date}</div>
        <div className="ft-mobile-prices" style={{ display:"none", gap:8, alignItems:"center", marginTop:6, paddingLeft:10, flexWrap:"wrap" }}>
          ...
          {item.status === "sold" && <><span style={{ color:"#444", fontSize:11 }}>→</span><span style={{ fontSize:13, fontWeight:700, color:GREEN }}>€{item.sale_price}</span></>}
      ...
      <div className="ft-price" style={{ color:item.status==="sold"?GREEN:"#2a2a4a" }}>{item.status==="sold"?`€${item.sale_price}`:"—"}</div>
      ...
      <span className="ft-platform-pill" style={{ background:`${buyPlatColor}20`, color:buyPlatColor, border:`1px solid ${buyPlatColor}40` }}>{item.purchase_platform||"—"}</span>
      <StatusPill status={item.status} />
  ...
  return (
    <div className="ft-overlay" onClick={onClose}>
      <div className="ft-modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="ft-modal-cat"><Tag size={11} />{item.category} · <StatusPill status={item.status} /></div>
        <div className="ft-modal-grid">
          <div className="ft-modal-box">
            ...
            <div className="ft-modal-box-sub">acquisto + spedizione</div>
          </div>
          <div className="ft-modal-box">
            ...
            <div className="ft-modal-box-sub">vendita - sped. - fee</div>
          </div>
          <div className="ft-modal-box">
          ...
          <div className="ft-modal-box">
            ...
            <div className="ft-modal-box-val" style={{color:roi!=null?YELLOW:"#2a2a4a"}}>{roi!=null?`${roi}%`:"—"}</div>
          </div>
        </div>
  ...
  return (
    <div className="ft-overlay" onClick={onClose}>
      <div className="ft-modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="ft-field"><label className="ft-label">Nome articolo *</label><input className="ft-input" placeholder="es. Nintendo 3DS XL" value={form.product_name} onChange={e=>set("product_name",e.target.value)} /></div>
        <div className="ft-2col">
          <div className="ft-field"><label className="ft-label">Categoria</label><select className="ft-input" style={{appearance:"none"}} value={form.category} onChange={e=>set("category",e.target.value)}><option value="videogame">Videogioco</option><option value="console">Console</option><option value="accessory">Accessorio</option></select></div>
          <div className="ft-field"><label className="ft-label">Stato</label><select className="ft-input" style={{appearance:"none"}} value={form.status} onChange={e=>set("status",e.target.value)}><option value="online">Online</option><option value="to_publish">Da pubblicare</option><option value="sold">Venduto</option></select></div>
        </div>
        <div className="ft-form-section">📥 Acquisto</div>
        <div className="ft-3col">
          <div className="ft-field"><label className="ft-label">Prezzo (€) *</label><input className="ft-input" type="number" placeholder="0.00" value={form.purchase_price} onChange={e=>set("purchase_price",e.target.value)} /></div>
          <div className="ft-field"><label className="ft-label">Spedizione (€)</label><input className="ft-input" type="number" placeholder="0.00" value={form.purchase_shipping} onChange={e=>set("purchase_shipping",e.target.value)} /></div>
          <div className="ft-field"><label className="ft-label">Piattaforma</label><select className="ft-input" style={{appearance:"none"}} value={form.purchase_platform} onChange={e=>set("purchase_platform",e.target.value)}><option>Subito</option><option>Vinted</option><option>eBay</option><option>Marketplace</option><option>Altra</option></select></div>
        </div>
        <div className="ft-field"><label className="ft-label">Data acquisto *</label><input className="ft-input" type="date" value={form.purchase_date} onChange={e=>set("purchase_date",e.target.value)} /></div>
        <div className="ft-form-section">📤 Vendita (opzionale)</div>
        <div className="ft-3col">
          <div className="ft-field"><label className="ft-label">Prezzo (€)</label><input className="ft-input" type="number" placeholder="0.00" value={form.sale_price} onChange={e=>set("sale_price",e.target.value)} /></div>
          <div className="ft-field"><label className="ft-label">Spedizione (€)</label><input className="ft-input" type="number" placeholder="0.00" value={form.sale_shipping} onChange={e=>set("sale_shipping",e.target.value)} /></div>
          <div className="ft-field"><label className="ft-label">Commissioni (€)</label><input className="ft-input" type="number" placeholder="0.00" value={form.fees} onChange={e=>set("fees",e.target.value)} /></div>
        </div>
        <div className="ft-2col">
          <div className="ft-field"><label className="ft-label">Piattaforma vendita</label><select className="ft-input" style={{appearance:"none"}} value={form.sale_platform} onChange={e=>set("sale_platform",e.target.value)}><option value="">— nessuna —</option><option>eBay</option><option>Vinted</option><option>Subito</option><option>Altra</option></select></div>
          <div className="ft-field"><label className="ft-label">Data vendita</label><input className="ft-input" type="date" value={form.sale_date} onChange={e=>set("sale_date",e.target.value)} /></div>
        </div>
        <div className="ft-form-section">🏷️ Listing (opzionale)</div>
        <div className="ft-2col">
          <div className="ft-field"><label className="ft-label">Prezzo listing (€)</label><input className="ft-input" type="number" placeholder="0.00" value={form.listing_price} onChange={e=>set("listing_price",e.target.value)} /></div>
          <div className="ft-field"><label className="ft-label">Data listing</label><input className="ft-input" type="date" value={form.listing_date} onChange={e=>set("listing_date",e.target.value)} /></div>
        </div>
        <div className="ft-field"><label className="ft-label">Note</label><textarea className="ft-textarea" placeholder="Dove l'hai trovato, prezzo obiettivo..." value={form.notes} onChange={e=>set("notes",e.target.value)} /></div>
        <button className="ft-save-btn" onClick={handleSave} disabled={saving}>
          {saving ? <><Loader2 size={16} className="ft-spin" />Salvataggio...</> : <><Plus size={16} />Salva articolo</>}
        </button>
      </div>
    </div>
  );
            ...
            <ResponsiveContainer width="100%" height={180}>
              <BarChart data={monthlyData} barSize={26}>
                ...
                <Bar dataKey="profit" radius={[6,6,0,0]}>
                  {monthlyData.map((_,i) => <Cell key={i} fill={ACCENT} fillOpacity={i===monthlyData.length-1?1:0.55} />)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
        ...
        <div className="ft-card">
          ...
          {cats.length ? cats.map(c => (
            <div className="ft-cat-item" key={c.cat}>
              ...
              <div className="ft-cat-bar-bg"><div className="ft-cat-bar-fill" style={{ width:`${Math.round((c.profit/maxProfit)*100)}%`, background:ACCENT }} /></div>
      ...
      <div className="ft">
        <aside className="ft-sidebar">
          ...
          <div className="ft-sidebar-footer">
            <button className="ft-add-btn" onClick={()=>setShowForm(true)}><Plus size={16} />Nuovo articolo</button>
          </div>
        </aside>
        ...
        <main className="ft-main">
          <div className="ft-page-header">
            ...
            <button className="ft-refresh-btn" onClick={load}><RefreshCw size={13} />Aggiorna</button>
          </div>
          {error && <div className="ft-error"><AlertCircle size={16}/>{error}</div>}
          {loading ? (
            <div className="ft-loading"><Loader2 size={32} color={ACCENT} className="ft-spin" /><span>Caricamento...</span></div>
          ) : (
